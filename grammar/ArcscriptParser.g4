...
input
  : script EOF
  | codestart compound_condition_or codeend EOF
  ;
...
paragraph
  : paragraph_start PARAGRAPHEND { this.currentLine++;}
  ;
...
assignment_segment
  : codestart statement_assignment codeend
  ;
...
if_section
  : codestart if_clause codeend script
  ;

else_if_section
  : codestart else_if_clause codeend script
  ;

else_section
  : codestart ELSEKEYWORD codeend script
  ;

if_clause
  : IFKEYWORD compound_condition_or
  ;

else_if_clause
  : ELSEIFKEYWORD compound_condition_or
  ;

endif_segment
  : codestart ENDIFKEYWORD codeend
  ;

statement_assignment
  : VARIABLE (ASSIGNADD | ASSIGNSUB | ASSIGNMUL | ASSIGNDIV | ASSIGN) compound_condition_or {this.assertVariable($VARIABLE);}
  ;
...
mention
  : MENTION_TAG_OPEN attr+=mention_attributes* '>'
    MENTION_LABEL? TAG_OPEN MENTION_TAG_CLOSE {this.assertMention($attr)}?
  ;

mention_attributes
  : ATTR_NAME (TAG_EQUALS ATTR_VALUE)?
  ;

additive_numeric_expression
  : multiplicative_numeric_expression ((ADD | SUB) additive_numeric_expression)?
  ;

multiplicative_numeric_expression
  : signed_unary_numeric_expression ((MUL | DIV) multiplicative_numeric_expression)?
  ;
...
compound_condition_or
  : compound_condition_and (( OR | ORKEYWORD) compound_condition_or)?
  ;

compound_condition_and
  : negated_unary_condition ((AND | ANDKEYWORD) compound_condition_and)?
  ;

negated_unary_condition
  : (NEG | NOTKEYWORD)? unary_condition
  ;
...
conditional_operator
  : GT | GE | LT | LE | EQ | NE | ISKEYWORD | ISKEYWORD NOTKEYWORD
  ;
