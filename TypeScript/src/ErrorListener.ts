import { RecognitionException, ErrorListener as BaseErrorListener, Recognizer, Token } from 'antlr4';
import { ParseError } from './errors/index.js';
import ArcscriptParser, { BlockquoteContext } from './Generated/ArcscriptParser.ts';
import ArcscriptLexer from './Generated/ArcscriptLexer.ts';
    ...
    if (recognizer instanceof ArcscriptParser) {
      if (e?.ctx.ruleContext instanceof BlockquoteContext) {
        if (
          ...
          recognizer.getTokenStream().get(offendingSymbol.tokenIndex - 1)?.type === ArcscriptLexer.BQ_CODESTART
        ) {
